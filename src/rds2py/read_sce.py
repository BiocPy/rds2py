...
def parse_single_cell_experiment(robject: dict):
    ...
    robj_altExps = None
    col_attrs = _dispatcher(
        robject["attributes"]["int_colData"]["attributes"]["listData"]["attributes"][
            "names"
        ]
    )
    ...
    for idx in range(len(col_attrs)):
        ...
        if idx_col == "reducedDims" and idx_value["data"] is not None:
            robj_reduced_dims = _dispatcher(
                robject["attributes"]["int_colData"]["attributes"]["listData"]["data"]
            )
